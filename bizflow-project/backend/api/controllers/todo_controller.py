...
# Initialize service with session after app context is available
todo_service = None
...
def list_todos():
    ...
    """
    todos = get_todo_service().list_todos()
              ...
              schema:
                ...
                properties:
                  message:
                    type: string
    """
    todo = get_todo_service().get_todo(todo_id)
    if not todo:
...
def create_todo():
    ...
    now = datetime.utcnow()
    todo = get_todo_service().create_todo(
        title=data['title'],
...
def update_todo(todo_id):
    ...
    if errors:
        return jsonify(errors), 400
    todo = get_todo_service().update_todo(
        todo_id=todo_id,
        title=data['title'],
        description=data['description'],
        status=data['status'],
              ...
              schema:
                ...
                properties:
                  message:
                    type: string
    """
    get_todo_service().delete_todo(todo_id)
    return '', 204
